...
/**
 * 通过Nacos下发动态路由配置，监听Nacos中 路由配置变更
 * @DependsOn 依赖于gatewayConfig这个Bean
...
public class DynamicRouteServiceImplByNacos {
    ...
    public void init(){
        ...
        try{
            ...
            // 反序列化成所需的对象
            JSONArray configInfoJsonArray = JSONUtil.parseArray(configInfo);
            List<RouteDefinition> definitionList = JSONUtil.toList(configInfoJsonArray, RouteDefinition.class);
        ...
        // 添加完成后，设置监听动态配置 --> 目的在于，当Nacos配置发生变更时，进行动态变更到Gateway配置上
    ...
    private void dynamicRouteNacosListener(String dataId, String group){
        try{
            // 给Nacos Config 客户端增加一个监听器,Listener实现一个匿名内部类
